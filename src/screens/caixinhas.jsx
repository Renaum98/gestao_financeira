...
const CORES_CAIXINHA = ['#6E4FF6', '#EF6B5C', '#1B9E6A', '#2566EA', '#E08A00', '#FF7AA8', '#3FCB9A', '#9B7BFF'];
  ...
  return (
    <div style={{ paddingBottom: 110 }}>
      ...
      <div style={{ padding: '4px 20px 0' }}>
        ...
        <button onClick={() => setModal('nova')} style={{
      ...
      {modal && (
        <ModalCaixinha
          ...
          onSalvar={(dados) => { salvarCaixinha(dados); setModal(null); }}
        />
      )}
    </div>
  );
...
export function CaixinhaScreen({ ctx, params }) {
  const { caixinhas, voltar, depositarCaixinha, excluirCaixinha, salvarCaixinha, ocultar } = ctx;
  ...
  return (
    <div style={{ paddingBottom: 110 }}>
      <TopBar voltar={voltar} acao={
        <button onClick={() => setModalEditar(true)} style={{
      ...
      <div style={{ padding: '4px 20px 0' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ marginTop: 14, fontSize: 12, fontWeight: 600, opacity: 0.85 }}>Você já juntou</div>
            ...
            {cx.meta > 0 && (
              <>
                <div style={{ marginTop: 12, height: 8, background: 'rgba(255,255,255,0.2)', borderRadius: 8, overflow: 'hidden' }}>
        ...
        {lembranca && (
          <Card style={{ marginTop: 14, padding: 16, display: 'flex', alignItems: 'flex-start', gap: 12 }}>
            <div style={{
              ...
              background: lembranca.completo ? '#DAF5E9' : lembranca.vencido ? '#FFE5EA' : 'color-mix(in oklab, var(--primary) 14%, transparent)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
              <Icon
                name={lembranca.completo ? 'check' : lembranca.vencido ? 'bell' : 'target'}
                size={18}
                color={lembranca.completo ? '#1B9E6A' : lembranca.vencido ? '#D63A55' : 'var(--primary)'}
                strokeWidth={2.4}
              />
            </div>
            <div style={{ flex: 1 }}>
              {lembranca.completo ? (
                <>
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#1B9E6A' }}>Meta alcançada! 🎉</div>
                ...
                <>
                  <div style={{ fontSize: 14, fontWeight: 800, color: '#D63A55' }}>Prazo vencido</div>
                  <div style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 500, marginTop: 2 }}>
                ...
                <>
                  <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--ink)' }}>
                    Guarde {fmtBRL(lembranca.valor, ocultar)} por {lembranca.tipo === 'mensal' ? 'mês' : lembranca.tipo === 'semanal' ? 'semana' : 'dia'}
                  </div>
                  <div style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 500, marginTop: 2 }}>
                    Para chegar em {rotuloDataCurto(lembranca.dataMeta)} ({lembranca.dias} {lembranca.dias === 1 ? 'dia restante' : 'dias restantes'}).
                  </div>
                </>
              )}
            </div>
        ...
        {/* CTA depositar */}
        <button onClick={() => setModalDeposito(true)} style={{
        ...
        {/* Histórico */}
        <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 0.4, padding: '24px 4px 8px' }}>
          {depositos.length === 0 ? 'Nenhum depósito ainda' : `${depositos.length} depósito${depositos.length === 1 ? '' : 's'}`}
        </div>
        {depositos.length > 0 && (
            ...
            {depositos.map((d, i) => (
              <div key={d.id} style={{
                ...
                <div style={{
                  ...
                  <Icon name="plus" size={16} color={cx.cor} strokeWidth={2.4} />
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)' }}>{fmtBRL(d.valor, ocultar)}</div>
        ...
        {/* Excluir */}
        <button onClick={() => setConfirmarExclusao(true)} style={{
          width: '100%', marginTop: 22, padding: '12px',
          borderRadius: 14, border: 'none', cursor: 'pointer',
          background: 'transparent', color: '#D63A55',
      ...
      {modalDeposito && (
        <ModalDeposito
          ...
          onSalvar={(dep) => { depositarCaixinha(cx.id, dep); setModalDeposito(false); }}
        />
      )}
      {modalEditar && (
        <ModalCaixinha
          ...
          onSalvar={(dados) => { salvarCaixinha({ ...cx, ...dados }); setModalEditar(false); }}
        />
      )}
      {confirmarExclusao && (
        <ConfirmModal
  ...
  return (
    <div onClick={onClick} style={{
      ...
      {lembranca && !lembranca.completo && !lembranca.vencido && !lembranca.semData && (
        <div style={{
          marginLeft: 4, marginTop: 10, padding: '8px 10px', borderRadius: 10,
          background: `${cx.cor}14`,
          fontSize: 11, fontWeight: 700, color: cx.cor,
          display: 'flex', alignItems: 'center', gap: 6,
        }}>
          <Icon name="target" size={12} color={cx.cor} strokeWidth={2.4} />
          Guarde {fmtBRLCompacto(lembranca.valor, ocultar)} por {lembranca.tipo === 'mensal' ? 'mês' : lembranca.tipo === 'semanal' ? 'semana' : 'dia'}
        </div>
      )}
    </div>
  );
...
function ModalCaixinha({ editando, onFechar, onSalvar }) {
  ...
  const [meta, setMeta] = React.useState(editando?.meta ? String(editando.meta).replace('.', ',') : '');
  ...
  return (
    <ModalShell titulo={editando ? 'Editar caixinha' : 'Nova caixinha'} onFechar={onFechar} onSalvar={salvar} salvarAtivo={valido}>
      <Campo label="Nome">
        <input
          autoFocus
          value={nome}
      ...
      <Campo label="Meta (opcional)">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: temMeta ? 10 : 0 }}>
          ...
          <>
            ...
            <div style={{ marginTop: 10 }}>
              <label style={{
                ...
                borderRadius: 12, background: 'var(--card-2)', boxShadow: '0 1px 2px rgba(0,0,0,0.06)',
              }}>
...
function ModalDeposito({ cor, onFechar, onSalvar }) {
  ...
  return (
    <ModalShell titulo="Adicionar valor" onFechar={onFechar} onSalvar={salvar} salvarAtivo={valorNum > 0} corAcento={cor}>
      <label style={{
        display: 'block', textAlign: 'center', padding: '14px 0 6px',
        position: 'relative', cursor: 'text',
      }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 0.6 }}>
          Valor
        </div>
        <div style={{ fontSize: 42, fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.03em', marginTop: 4, fontVariantNumeric: 'tabular-nums' }}>
          <span style={{ fontSize: 20, color: 'var(--muted)', marginRight: 6, verticalAlign: 'top' }}>R$</span>
          {valor}
        </div>
        <input
          autoFocus
...
function ModalShell({ titulo, onFechar, onSalvar, salvarAtivo, corAcento, children }) {
  return (
    <div onClick={onFechar} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        width: '100%', maxWidth: 440, maxHeight: 'calc(100vh - 40px)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--ink)', letterSpacing: '-0.01em' }}>{titulo}</div>
          <button onClick={onSalvar} disabled={!salvarAtivo} style={{
            ...
            fontWeight: 800, fontSize: 13, cursor: salvarAtivo ? 'pointer' : 'default',
            fontFamily: 'inherit',
          }}>Salvar</button>
        </div>
        {children}
...
function Campo({ label, children }) {
  return (
    <div style={{ marginTop: 14 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: 0.4, padding: '0 4px 6px' }}>
        {label}
      </div>
      {children}
    </div>
...
const inputStyle = {
  ...
  border: 'none', background: 'var(--card-2)', outline: 'none',
