...
import { MESES_CURTO, fmtBRLCompacto, rotuloMes, rotuloMesCurto, totalGeral, txDoMes } from '../data.js';
import { Icon } from '../ui/icons.jsx';
import { Card, TopBar } from '../ui/common.jsx';

export function HistoricoScreen({ ctx }) {
  const { txs, todosMeses, ocultar, voltar, setMes, irPara, mesAnterior, mes } = ctx;
    ...
    <div style={{ paddingBottom: 110 }}>
      ...
      <div style={{ padding: '20px 20px 0' }}>
        ...
        <Card style={{ padding: '4px 16px' }}>
          {dadosMeses.map((d, i) => (
            <div key={d.mes} onClick={() => { setMes(d.mes); voltar(); }} style={{
              ...
              <div style={{
                ...
                <div style={{ fontSize: 13, letterSpacing: '-0.02em' }}>{MESES_CURTO[parseInt(d.mes.split('-')[1], 10) - 1]}</div>
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)' }}>{rotuloMes(d.mes)}</div>
                <div style={{ fontSize: 12, color: 'var(--muted)', fontWeight: 600, marginTop: 2 }}>{d.count} transações</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--ink)' }}>{fmtBRLCompacto(d.total, ocultar)}</div>
              </div>
