      ...
      >
        ...
        <div style={{ marginTop: 10 }}>
          <label style={{
            ...
            borderRadius: 14, background: 'var(--card-2)', boxShadow: '0 1px 2px rgba(0,0,0,0.06)',
          }}>
          ...
          {PAGAMENTOS.map(p => {
            ...
            return (
              <button key={p} onClick={() => setPagamento(p)} style={{
                ...
                <Icon name={iconePagamento(p)} size={18} color={sel ? 'var(--bg)' : 'var(--ink)'} strokeWidth={2} />
                {p.replace('Cartão de ', '')}
              </button>
            );
          })}
        ...
        {ehCredito && (
          <div style={{ marginTop: 10 }}>
            ...
            <div className="carrossel" style={{ display: 'flex', gap: 6, overflowX: 'auto', scrollbarWidth: 'none', paddingBottom: 2 }}>
              {[1, 2, 3, 4, 6, 10, 12].map(n => {
                ...
                return (
                  <button key={n} onClick={() => setParcelas(n)} style={{
                    ...
                    boxShadow: sel ? '0 2px 8px rgba(110,79,246,0.25)' : '0 1px 2px rgba(0,0,0,0.06)',
        ...
        {numParcelas === 1 && !editar && (
          <label style={{
            ...
            background: 'var(--card-2)', boxShadow: '0 1px 2px rgba(0,0,0,0.06)',
            cursor: 'pointer',
          }}>
            <div style={{
              width: 36, height: 36, borderRadius: 12,
              background: ehRecorrente ? 'color-mix(in oklab, var(--primary) 14%, transparent)' : 'var(--surface-sunken)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              flexShrink: 0, transition: 'background .15s',
            }}>
              <Icon name="history" size={18} color={ehRecorrente ? 'var(--primary)' : 'var(--muted)'} strokeWidth={2.2} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--ink)' }}>Repetir todo mês</div>
              <div style={{ fontSize: 11, color: 'var(--muted)', fontWeight: 500, marginTop: 1, lineHeight: 1.35 }}>
                Útil para assinaturas, aluguel, mensalidades. Você pode cancelar depois em Perfil → Recorrentes.
              </div>
            </div>
...
function ToggleSimples({ ativo, onChange }) {
  return (
    <div onClick={(e) => { e.preventDefault(); onChange(!ativo); }} style={{
